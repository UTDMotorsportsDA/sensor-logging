...
// Date: 4/4/2017
// ledScreenDisplays - JNI C++ functions that display 1 of 3 screens needed for UTD SAE instrument cluster.
...
static const Color color1(200, 100, 0);
static const Color color2(0, 100, 150);
static const char *bdf_font_file = "6x9.bdf";
static Font font;
...
Canvas *createCanvasForLedDisplay()
{
...
void displaySpeedAndRpm(int speed, int rpm, Canvas *canvas)
{
...
void displayTemperature(int temp, Canvas *canvas)
{
	signal(SIGTERM, InterruptHandler);
	signal(SIGINT, InterruptHandler);
	canvas->Clear();
...
void displayGForceGrid(int x, int y, Canvas *canvas)
{
	signal(SIGTERM, InterruptHandler);
	signal(SIGINT, InterruptHandler);
	canvas->Clear();
